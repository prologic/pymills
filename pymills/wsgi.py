...
(Taken from CherryPy: http://www.cherrypy.org)
...
For SSL support, set these attributes:
...
class SSL_fileobject(fileobject):
...
class HTTPConnection(object):
	...
	def __init__(self, sock, wsgi_app, environ):
		...
		if SSL and isinstance(sock, SSL.ConnectionType):
			...
			self.rfile = fileobject(sock, "rb", self.rbufsize)
			self.wfile = fileobject(sock, "wb", -1)
	...
	def communicate(self):
		...
		try:
			...
			# Unwrap our wfile
			req.wfile = fileobject(self.socket, "wb", -1)
...
class ThreadPool(object):
	"""A Request Queue for the WSGIServer which pools threads.
	...
	def start(self):
		...
		for worker in self._threads:
			worker.setName("WSGIServer " + worker.getName())
			worker.start()
		for worker in self._threads:
...
class WSGIServer(object):
	...
	version = "pymills/%s" % pymills.__version__
	ready = False
	_interrupt = None
